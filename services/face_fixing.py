...
class FaceFixingPipeline:
    ...
    def _load_face_detector(self) -> None:
        """Load face detection model using OpenCV (more stable than MediaPipe)."""
        if self.face_detector is not None:
            return

        try:
            print('[FaceFixing] Loading face detector (OpenCV cascade)...')
            import cv2

            # Use OpenCV's pre-trained face detector - more stable and reliable
            cascade_path = cv2.data.haarcascades + 'haarcascade_frontalface_default.xml'
            self.face_detector = cv2.CascadeClassifier(cascade_path)

            if self.face_detector.empty():
                raise RuntimeError('Failed to load OpenCV cascade classifier')

            print('[FaceFixing] Face detector loaded (OpenCV cascade)')
    ...
    def _detect_faces(self, image: np.ndarray) -> list:
        ...
        faces_cv = self.face_detector.detectMultiScale(
        ...
        for x, y, w, h in faces_cv:
            x2 = x + w
            y2 = y + h
            # Normalize to 0-1 confidence (use 0.9 as default since cascade doesn't return confidence)
            faces.append({'box': (x, y, x2, y2), 'score': 0.9})
