/**
 * Test: Persephone Model with Local Flux .1 Dev Encoders
 *
 * Validates that:
...
test('🔴 RED: Persephone Model with Local Flux .1 Dev Encoders', async (t) => {
  await t.test('Encoder Files Exist', async (t) => {
    ...
    await t.test('Persephone checkpoint file exists', () => {
      const modelPath = path.join(checkpointsDir, 'persephoneFluxNSFWSFW_11FP8.safetensors');
      assert.strictEqual(fs.existsSync(modelPath), true, `Persephone model not found at ${modelPath}`);
    });
  });
  ...
  await t.test('Environment Configuration', async (t) => {
    await t.test('Can configure environment for local encoders', () => {
      const envConfig = {
        FLUX_MODEL_PATH: path.join(checkpointsDir, 'persephoneFluxNSFWSFW_11FP8.safetensors'),
    ...
    if (!serviceStartFailed) {
      ...
      await t.test('Generates image without shape mismatch errors', async () => {
        const response = await httpRequest(fluxPort, '/generate', {
          model: path.join(checkpointsDir, 'persephoneFluxNSFWSFW_11FP8.safetensors'),
