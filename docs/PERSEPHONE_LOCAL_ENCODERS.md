# Persephone Model with Local Flux .1 Dev Encoders

This document describes how to use the Persephone Flux model with locally-downloaded Flux .1 Dev encoders to avoid shape mismatch errors.

## Problem Statement

Weight-only Flux checkpoints like Persephone contain only the transformer weights and lack text encoders (CLIP-L) and VAE. The service attempted to load fallback encoders from HuggingFace, which caused tensor shape mismatches:
...
The Persephone checkpoint goes in `services/checkpoints/`:
- `persephoneFluxNSFWSFW_11FP8.safetensors` (~10.7 GB)
...
FLUX_MODEL_PATH=services/checkpoints/persephoneFluxNSFWSFW_11FP8.safetensors

# Local text encoders (required for Persephone)
...
export FLUX_MODEL_PATH=$(pwd)/services/checkpoints/persephoneFluxNSFWSFW_11FP8.safetensors
...
Wait for output:
```
[Flux Service] Using local model: /path/to/persephoneFluxNSFWSFW_11FP8.safetensors
...
- Persephone Model: https://civitai.com/models/...
